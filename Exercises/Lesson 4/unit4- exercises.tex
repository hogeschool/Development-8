...
\exercise{
  \functionEx{curry}{<a, b, c>(f: Tuple<a, b> => c) => (x: a) => (y: b): c}{
...
\exercise{
  \functionEx{flatten}{<a>(l: List<List<a>>): List<a>}{
    that takes a list of lists and places all their elements into a single one. Use \texttt{fold} to implement this function.
}}\\
